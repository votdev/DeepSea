...
def add_users(pathname="/srv/salt/ceph/rgw/cache", jinja="/srv/salt/ceph/rgw/files/users.j2"):
    ...
    """
    _users = __salt__['slsutil.renderer'](jinja)
    log.debug("users rendered: {}".format(_users))

    if _users is None or 'realm' not in _users:
        return

    for realm in _users['realm']:
        for user in _users['realm'][realm]:
