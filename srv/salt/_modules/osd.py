...
def _filter_devices(_devices, **kwargs):
    ...
    """
    if 'format' in kwargs:
        _filtered = [device for device in _devices
    ...
    def _bluestore_args(self):
        ...
        if self.osd.wal and self.osd.db:
            if self.is_partitioned(self.osd.wal):
                ...
                # already refactored in another PR
            ...
            if self.osd.wal:
                ...
                if self.is_partitioned(self.osd.wal):
                    ...
                    if self.osd.wal == self.osd.device:
                        log.warn(("Separate wal partition on {} unnecessary - "
                                  "triggers ceph-disk bug".format(self.osd.wal)))
                    else:
            ...
            if self.osd.db:
                if self.is_partitioned(self.osd.db):
                    ...
                    if self.osd.db == self.osd.device:
                        log.warn(("Separate db partition on {} unnecessary - "
                                  "triggers ceph-disk bug".format(self.osd.db)))
                    else:
